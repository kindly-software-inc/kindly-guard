...
use kindly_guard_server::{
    neutralizer::{
        create_neutralizer, NeutralizeAction, NeutralizeResult, ThreatNeutralizer,
    },
    scanner::{
        create_scanner, Location, SecurityScanner, Severity, Threat, ThreatType,
    },
    ...
    vec![
        ...
        (
            Threat {
                ...
                location: Location::Text { offset: 35, length: 37 },
        ...
        (
            Threat {
                ...
                severity: Severity::Critical,
                location: Location::Text { offset: 5, length: 13 },
        ...
        (
            Threat {
                ...
                location: Location::Text { offset: 0, length: 8 },
        ...
        (
            Threat {
                ...
                severity: Severity::High,
                location: Location::Text { offset: 0, length: 17 },
        ...
        (
            Threat {
                ...
                severity: Severity::High,
                location: Location::Text { offset: 0, length: 50 },
        ...
        (
            Threat {
                threat_type: ThreatType::SqlInjection,
                severity: Severity::Critical,
                location: Location::Text { offset: 0, length: 0 }, // Empty location
        ...
        (
            Threat {
                ...
                location: Location::Text { offset: 1000, length: 10 }, // Out of bounds
    ...
    vec![
        ...
        (
            Threat {
                threat_type: ThreatType::SqlInjection,
                severity: Severity::Critical,
                location: Location::Text { offset: 0, length: 10000 },
...
async fn test_scanner_behavioral_equivalence() {
    ...
    for input in test_inputs {
        let std_threats = standard.scan_text(input).await
            .expect("Standard scanning should not fail");
        let enh_threats = enhanced.scan_text(input).await
            .expect("Enhanced scanning should not fail");
        
        // Same number of threats detected
        assert_eq!(
            std_threats.len(), enh_threats.len(),
        ...
        for (std_threat, enh_threat) in std_sorted.iter().zip(enh_sorted.iter()) {
            ...
            match (&std_threat.location, &enh_threat.location) {
                (Location::Text { offset: o1, length: l1 }, 
                 Location::Text { offset: o2, length: l2 }) => {
...
async fn test_circuit_breaker_behavioral_equivalence() {
    ...
    for _ in 0..5 {
        let std_result = standard.call("test", || async { 
            Err::<i32, _>("Simulated failure".to_string()) 
        }).await;
        let enh_result = enhanced.call("test", || async { 
            Err::<i32, _>("Simulated failure".to_string()) 
        }).await;
    ...
    let std_open = standard.call("test", || async { Ok::<_, String>(42) }).await;
    let enh_open = enhanced.call("test", || async { Ok::<_, String>(42) }).await;
...
async fn test_retry_handler_behavioral_equivalence() {
    ...
    let std_result = standard.retry("test", || async { Ok::<_, String>(42) }).await;
    let enh_result = enhanced.retry("test", || async { Ok::<_, String>(42) }).await;
    ...
    let std_result = standard.retry("test", || {
        let c = counter.clone();
        async move {
    ...
    let enh_result = enhanced.retry("test", || {
...
async fn test_complex_scenario_equivalence() {
    ...
    for (std_threat, enh_threat) in std_threats.iter().zip(enh_threats.iter()) {
        let std_result = std_neutralizer.neutralize(std_threat, content).await.unwrap();
        let enh_result = enh_neutralizer.neutralize(enh_threat, content).await.unwrap();
...
async fn test_performance_characteristics_documented() {
    ...
    let threat = Threat {
        threat_type: ThreatType::SqlInjection,
        severity: Severity::High,
        location: Location::Text { offset: 0, length: 1000 },
    ...
    let small_threat = Threat {
        ...
        location: Location::Text { offset: 0, length: 5 },
