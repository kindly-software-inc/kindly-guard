...
async fn test_e2e_complete_security_workflow() {
    ...
    let response = server
        .handle_message(&list_tools.to_string())
        .await
        .unwrap();
    ...
    let response = server
        .handle_message(&scan_request.to_string())
        .await
        .unwrap();
    let scan_response: Value = serde_json::from_str(&response).unwrap();
    let content = scan_response["result"]["content"][0]["text"]
        .as_str()
        .unwrap();
    ...
    let response = server
        .handle_message(&status_request.to_string())
        .await
        .unwrap();
    ...
    let response = server
        .handle_message(&shield_request.to_string())
        .await
        .unwrap();
    let shield_response: Value = serde_json::from_str(&response).unwrap();
    let shield_content = shield_response["result"]["content"][0]["text"]
        .as_str()
        .unwrap();
...
async fn test_e2e_unicode_attack_scenario() {
    ...
    for (i, (text, description)) in unicode_attacks.iter().enumerate() {
        ...
        assert!(
            !result["threats"].as_array().unwrap().is_empty(),
...
async fn test_e2e_error_recovery_scenario() {
    ...
    let response = server
        .handle_message(&valid_request.to_string())
        .await
        .unwrap();
    let response_json: Value = serde_json::from_str(&response).unwrap();
...
async fn test_e2e_performance_under_load() {
    ...
    assert!(
        success_count > request_count * 90 / 100,
    ...
    println!("Processed {success_count} requests in {elapsed:?}");
}
