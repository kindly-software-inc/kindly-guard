...
use kindly_guard_server::{
    // Core traits
    traits::{
        ThreatScanner, ThreatNeutralizer, RateLimiter, CircuitBreakerTrait,
        AuditLogger, Storage, Telemetry,
    },
    // Types
    ...
    error::KindlyError,
    // Component creation
    create_scanner, create_neutralizer, create_rate_limiter, 
    create_circuit_breaker, create_audit_logger, create_storage,
...
async fn test_multi_threat_scenario() {
    ...
    assert!(threat_types.iter().any(|t| matches!(t, ThreatType::XssAttempt { .. })));
    ...
    for threat in &threats {
        let result = neutralizer.neutralize(threat, malicious_content).await.unwrap();
...
async fn test_circuit_breaker_activation_and_recovery() {
    ...
    let failing_op = || async {
        Err::<(), KindlyError>(KindlyError::Internal("Service failure".into()))
    };
    
    // Helper for successful operation
    let success_op = || async {
        Ok::<&str, KindlyError>("Success")
    };
...
async fn test_audit_logging_integration() {
    ...
    audit_logger.log_threat_neutralized(&threat, &neutralize_result).await.unwrap();
    ...
    // Verify log content
    let detection_log = logs.iter().find(|l| l.event_type == "threat_detected").unwrap();
    assert_eq!(detection_log.threat_type, Some("SqlInjection".to_string()));
    assert_eq!(detection_log.severity, Some(Severity::High));
    
    let neutralization_log = logs.iter().find(|l| l.event_type == "threat_neutralized").unwrap();
    assert_eq!(neutralization_log.action_taken, Some("Sanitize".to_string()));
}
...
async fn test_distributed_tracing_integration() {
    ...
    assert!(trace.duration_ms >= 80, "Total duration should be at least 80ms");
}
...
async fn test_websocket_transport_integration() {
    ...
    let transport = create_transport(TransportConfig::WebSocket {
...
async fn test_http_transport_integration() {
    ...
    let transport = create_transport(TransportConfig::Http {
        host: "127.0.0.1".to_string(),
        port: 0,
        path: "/mcp".to_string(),
    }, &config).await.unwrap();
