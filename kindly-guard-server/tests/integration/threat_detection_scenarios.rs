...
async fn test_combined_sql_unicode_attack() {
    ...
    for attack in attacks {
        ...
        let threat_types: Vec<_> = threats.iter().map(|t| t.threat_type.as_str()).collect();

        assert!(
            threat_types
                .iter()
                .any(|t| t.contains("sql") || t.contains("injection")),
...
async fn test_command_injection_attacks() {
    ...
    for attack in command_attacks {
        ...
        assert!(
            threats[0].threat_type.contains("command")
                || threats[0].threat_type.contains("injection"),
...
async fn test_threat_neutralization() {
    ...
    let test_cases = vec![
        ...
        ("'; DROP TABLE users; --", "SQL context"),
        ("p\u{0430}ypal.com", "Unicode homograph"),
