...
async fn test_basic_neutralizer() {
    ...
    assert!(
        neutralized.sanitized_content.is_some() || neutralized.extracted_params.is_some(),
...
fn test_json_scanning() {
    let scanner = SecurityScanner::new(ScannerConfig::default()).expect("Failed to create scanner");
    ...
    let threat_types: Vec<_> = threat_list.iter().map(|t| &t.threat_type).collect();
...
fn test_large_input_performance() {
    ...
    assert!(
        duration.as_secs() < 2,
        "Large input scan took too long: {:?}",
        duration
    );
...
fn test_concurrent_scanning() {
    let scanner = std::sync::Arc::new(
        SecurityScanner::new(ScannerConfig::default()).expect("Failed to create scanner"),
    );
...
fn test_mixed_content() {
    ...
    let threat_types: std::collections::HashSet<_> =
        threats.iter().map(|t| &t.threat_type).collect();

    assert!(
        threat_types.len() >= 2,
        "Should detect different threat types"
    );
}
