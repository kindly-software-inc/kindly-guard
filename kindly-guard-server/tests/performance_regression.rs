use kindly_guard_server::{
    config::Config,
    neutralizer::{
        NeutralizeAction, NeutralizeResult, ThreatNeutralizer, ThreatNeutralizerError,
    },
...
impl RegressionDetector {
    ...
    fn detect_regression(
        ...
        let combined_std_dev = (current.std_dev.as_nanos() as f64 + baseline.std_deviation_ns as f64) / 2.0;
...
impl BenchmarkRunner {
    ...
    fn measure_neutralization<N: ThreatNeutralizer + Send + Sync + 'static>(
        ...
        for threat in threats.iter().cycle().take(self.warmup_iterations) {
            let _ = self.runtime.block_on(neutralizer.neutralize(threat, content));
        }
        ...
        for threat in threats.iter().cycle().take(self.measurement_iterations) {
            ...
            let _ = self.runtime.block_on(async move {
                neutralizer.neutralize(&threat, &content).await
            });
    ...
    fn measure_scanning<S: SecurityScanner + Send + Sync + 'static>(
        ...
        for content in content_samples.iter().cycle().take(self.measurement_iterations) {
...
mod tests {
    ...
    fn test_large_content_performance() {
        ...
        let measurement = runner.measure_batch_operation(
            || scanner.scan_text(&large_content),
            1,
        );
    ...
    fn test_batch_neutralization_performance() {
        ...
        if let Some(baseline) = baselines.get(&key) {
            ...
            baselines.insert(key, measurement.to_baseline("batch_neutralization", "standard"));
            detector.save_baselines(&baselines);
        }
    }
    ...
    fn test_statistical_significance() {
        ...
        assert!(detector.detect_regression(&normal_variation, &baseline).is_none());
