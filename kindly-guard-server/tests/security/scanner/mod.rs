...
mod scanner_security_tests {
    ...
    async fn test_sql_injection_detection() {
        ...
        for pattern in sql_patterns {
            ...
            if !result.is_empty() {
                ...
                // Verify threat level
                assert_eq!(
                    threat.severity, kindly_guard_server::scanner::Severity::Critical,
        ...
        for pattern in unicode_patterns {
            ...
            if pattern.expected_detection {
                ...
                assert!(
                    matches!(
                        &threat.threat_type,
                        ThreatType::UnicodeInvisible | ThreatType::UnicodeBiDi | 
                        ThreatType::UnicodeHomograph | ThreatType::UnicodeControl
                    ),
    ...
    async fn test_comprehensive_attack_coverage() {
        ...
        println!("  Overall detection rate: {:.2}%", overall_detection_rate * 100.0);
        println!("  Critical detection rate: {:.2}%", critical_detection_rate * 100.0);
    }
            ...
            b.iter(|| {
                rt.block_on(async {
                    for pattern in &patterns {
                        let _ = SecurityScannerTrait::scan_text(&*scanner, black_box(&pattern.payload));
                    }
                })
            })
        });
