...
mod auth_security_tests {
    ...
    async fn test_jwt_attack_patterns() {
        ...
        for attack in attack_patterns.patterns {
            ...
            let result = auth_manager
                .authenticate(Some(&format!("Bearer {}", attack.token)))
                .await;
    ...
    async fn test_timing_attack_resistance() {
        ...
        for _ in 0..test_config.timing_attack_iterations {
            let start = Instant::now();
            let _ = auth_manager
                .authenticate(Some(&format!("Bearer {}", invalid_token)))
                .await;
    ...
    async fn test_permission_enforcement() {
        ...
        assert!(auth_context.authenticated, "Token should be authenticated");
        assert!(
            auth_context
                .scopes
                .iter()
                .all(|s| !s.contains("write") && !s.contains("delete")),
    ...
    async fn test_cryptographic_security() {
        ...
        let weak_secrets = vec!["password", "123456", "secret", "12345678", "qwerty"];
