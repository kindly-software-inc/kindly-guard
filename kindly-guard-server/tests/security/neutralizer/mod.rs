...
use kindly_guard_server::{
    neutralizer::{
        create_neutralizer, create_neutralizer_with_telemetry, NeutralizeAction,
        NeutralizationConfig, NeutralizationMode, NeutralizeResult,
    },
...
impl NeutralizationMetrics {
    ...
    fn print_summary(&self) {
        ...
        println!("  Successfully neutralized: {}", self.successfully_neutralized);
        ...
        println!("  Average time: {:.2}ms", self.average_neutralization_time_ms);
        println!("  Rollback success rate: {}/{}", 
...
mod neutralizer_security_tests {
    ...
    async fn setup_test_environment(mode: NeutralizationMode) -> (Arc<SecurityScanner>, Arc<dyn ThreatNeutralizer>) {
    ...
    async fn test_sql_injection_neutralization() {
        ...
        for pattern in sql_patterns {
            ...
            if !threats.is_empty() {
                ...
                metrics.average_neutralization_time_ms = 
                    (metrics.average_neutralization_time_ms * (metrics.successfully_neutralized as f64) + elapsed) 
                ...
                let post_scan = scanner.scan_text(result.sanitized_content.as_ref().unwrap()).unwrap();
    ...
    async fn test_xss_neutralization() {
        ...
        for pattern in xss_patterns {
            ...
            for threat in threats {
                ...
                let result = neutralizer.neutralize(&threat, &pattern.payload).await.unwrap();
                
                if result.sanitized_content.is_some() {
                    // Verify no XSS remains
                    let post_scan = scanner.scan_text(result.sanitized_content.as_ref().unwrap()).unwrap();
                    let has_xss = post_scan.iter().any(|t| {
                        matches!(t.threat_type, ThreatType::CrossSiteScripting)
                    });
        ...
        for pattern in unicode_patterns {
            ...
            for threat in threats {
                ...
                if result.sanitized_content.is_some() {
                    ...
                    let contains_dangerous = dangerous_chars.iter()
                        .any(|&c| neutralized.contains(c));
        ...
        for pattern in prompt_patterns {
            ...
            for threat in threats {
                ...
                if result.sanitized_content.is_some() {
                    ...
                    let properly_handled = dangerous_phrases.iter()
                        .all(|phrase| {
    ...
    async fn test_neutralization_validation() {
        ...
        for (content, threat_type) in test_cases {
            ...
            assert!(result.is_ok(), "Neutralization should not panic on edge cases");
        }
    }
    ...
    async fn test_neutralization_performance() {
        ...
        for _ in 0..iterations {
            for pattern in &test_patterns {
                ...
                for threat in threats {
                    let _ = neutralizer.neutralize(&threat, &pattern.payload).await.unwrap();
    ...
    async fn test_combined_attack_neutralization() {
        ...
        for (payload, expected_threats) in combined_attacks {
            ...
            for threat in threats {
                ...
                let result = neutralizer.neutralize(&threat, &fully_neutralized).await.unwrap();
