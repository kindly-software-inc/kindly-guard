...
async fn test_jsonrpc_parse_errors() {
    ...
    for (input, description) in test_cases {
        ...
        let response_json: Value = serde_json::from_str(&response.unwrap())
            .expect(&format!("Should return valid JSON error for: {}", description));
...
async fn test_jsonrpc_invalid_request_errors() {
    ...
    let test_cases = vec![
        ...
        (json!({"jsonrpc": "2.0", "method": 123}), "Non-string method"),
        (json!({"jsonrpc": "2.0", "method": "test", "params": "invalid"}), "String params"),
        (json!({"jsonrpc": "2.0", "method": "test", "id": "invalid"}), "String id for number"),
    ];
    
    for (input, description) in test_cases {
        let response = server.handle_message(&input.to_string()).await
            .expect(&format!("Should return response for: {}", description));
        
        let response_json: Value = serde_json::from_str(&response)
            .expect(&format!("Should return valid JSON for: {}", description));
        
        assert!(response_json["error"].is_object(), 
            "Should have error for: {}", description);
        assert!(response_json["error"]["code"].as_i64().unwrap() < 0,
...
async fn test_method_not_found_errors() {
    ...
    for (i, method) in test_methods.iter().enumerate() {
        ...
        let response = server.handle_message(&request.to_string()).await
            .expect(&format!("Should return response for method: {}", method));
        
        let response_json: Value = serde_json::from_str(&response)
            .expect(&format!("Should return valid JSON for method: {}", method));
...
async fn test_batch_request_errors() {
    ...
    let response = server.handle_message(&empty_batch.to_string()).await
    ...
    let response = server.handle_message(&invalid_batch.to_string()).await
    ...
    assert!(responses.len() >= 2, "Should have responses for valid items");
}
...
async fn test_notification_errors() {
    ...
    let response = server.handle_message(&invalid_notification.to_string()).await;
    
    // Should return error since null id makes it a request, not notification
    assert!(response.is_some(), "Should return error for invalid notification");
}
...
async fn test_oversized_id_handling() {
    ...
    let response = server.handle_message(&large_id_request.to_string()).await
