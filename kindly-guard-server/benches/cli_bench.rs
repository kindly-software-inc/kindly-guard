...
use kindly_guard_server::cli::validation::{
    validate_file_path, validate_scan_input, validate_port, 
    validate_feature_name, sanitize_output
};
...
fn benchmark_path_validation(c: &mut Criterion) {
    ...
    for path in test_paths {
        group.bench_with_input(
            ...
            |b, path| {
                b.iter(|| {
                    black_box(validate_file_path(path).ok())
                });
            },
        );
    }
...
fn benchmark_input_validation(c: &mut Criterion) {
    ...
    let test_inputs = vec![
        ...
        ("unicode", "Unicode: 你好世界 مرحبا بالعالم"),
    ];
    ...
    for (label, input) in test_inputs {
        group.bench_with_input(
            ...
            |b, input| {
                b.iter(|| {
                    black_box(validate_scan_input(input).ok())
                });
            },
        );
    }
...
fn benchmark_output_sanitization(c: &mut Criterion) {
    ...
    let test_outputs = vec![
        ("plain", "Simple text without special characters"),
        ("ansi", "\x1b[31mRed text\x1b[0m with \x1b[32mgreen\x1b[0m colors"),
        ("control", "Text with\x00null\x01and\x02other\x07control chars"),
        ("mixed", "Mixed \x1b[35mcolors\x1b[0m and\x00\x01control\nchars\ttabs"),
        ("large", &format!("{}\x1b[31m{}\x1b[0m{}", 
            "x".repeat(1000), "colored", "x".repeat(1000))),
    ];
    
    for (label, output) in test_outputs {
        group.bench_with_input(
            BenchmarkId::new("sanitize", label),
            &output,
            |b, output| {
                b.iter(|| {
                    black_box(sanitize_output(output))
                });
            },
        );
    }
...
fn benchmark_feature_validation(c: &mut Criterion) {
    ...
    for feature in test_features {
        group.bench_function(BenchmarkId::new("validate", feature), |b| {
            b.iter(|| {
                black_box(validate_feature_name(feature).ok())
            });
        });
    }
...
fn benchmark_command_parsing(c: &mut Criterion) {
    ...
    let test_commands = vec![
        ...
        ("scan_text", vec!["/kindlyguard", "scan", "test input", "--text"]),
        ("scan_file", vec!["/kindlyguard", "scan", "/path/to/file.json"]),
        ("telemetry", vec!["/kindlyguard", "telemetry", "--detailed"]),
        ("advanced", vec!["/kindlyguard", "advancedsecurity", "enable"]),
        ("info", vec!["/kindlyguard", "info", "unicode"]),
        ("dashboard", vec!["/kindlyguard", "dashboard", "--port", "8080"]),
    ];
    ...
    for (label, args) in test_commands {
        group.bench_with_input(
            ...
            |b, args| {
                b.iter(|| {
                    black_box(KindlyCommand::try_parse_from(args).ok())
                });
            },
        );
    }
...
fn benchmark_validation_error_cases(c: &mut Criterion) {
    ...
    group.bench_function("invalid_path", |b| {
        b.iter(|| {
            black_box(validate_file_path("../../../etc/passwd").err())
        });
    });
    ...
    group.bench_function("oversized_input", |b| {
        ...
        b.iter(|| {
            black_box(validate_scan_input(&large_input).err())
        });
    });
    
    group.bench_function("invalid_port", |b| {
        b.iter(|| {
            black_box(validate_port(80).err())
        });
    });
