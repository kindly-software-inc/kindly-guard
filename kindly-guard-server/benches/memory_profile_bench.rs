...
use kindly_guard_server::{
    ...
    metrics::{MetricsRegistry, KindlyMetrics},
};
use std::sync::Arc;
            ...
            group.bench_function(
                ...
                |b| {
                    b.iter_custom(|iters| {
                        ...
                        eprintln!("{} {}: ~{} bytes per scan", mode, size_name, memory_per_iter);
        ...
        group.bench_function(
            ...
            |b| {
                b.iter_custom(|iters| {
                    ...
                    eprintln!("{} tasks: ~{} KB per iteration", num_tasks, memory_per_iter / 1024);
    ...
    group.bench_function("registry_with_metrics", |b| {
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                ...
                let metrics = KindlyMetrics::new(registry.clone());
            ...
            eprintln!("Metrics registry: ~{} KB per instance", memory_per_iter / 1024);
...
fn bench_server_lifecycle_memory(c: &mut Criterion) {
    ...
    for mode in &["standard", "enhanced"] {
        ...
        group.bench_function(
            BenchmarkId::from_parameter(format!("{}_mode", mode)),
            |b| {
                b.iter_custom(|iters| {
                    let (start_allocated, _) = get_memory_stats();
                    ...
                    eprintln!("{} server lifecycle: ~{} KB per create/destroy cycle", 
                        mode, memory_per_iter / 1024);
            ...
            |b| {
                b.iter_custom(|iters| {
                    ...
                    if memory_samples.len() > 2 {
                        ...
                        eprintln!("{} mode - Memory growth: {:.2} KB total, {:.2} bytes/iteration", 
                            mode, growth / 1024.0, growth_per_iter);
