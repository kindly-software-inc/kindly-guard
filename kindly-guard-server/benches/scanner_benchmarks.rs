...
fn throughput_benchmarks(c: &mut Criterion) {
    ...
    for size in [KB, 10 * KB, 100 * KB, MB].iter() {
        ...
        group.bench_function(format!("clean_text_{}", size), |b| {
            b.iter(|| {
                let threats = scanner
                    .scan_text(black_box(&clean_text))
                    .unwrap_or_default();
                black_box(threats);
            })
        });
        ...
        group.bench_function(format!("unicode_threats_{}", size), |b| {
            b.iter(|| {
                let threats = scanner
                    .scan_text(black_box(&unicode_text))
                    .unwrap_or_default();
                black_box(threats);
            })
        });
        ...
        group.bench_function(format!("injection_patterns_{}", size), |b| {
            b.iter(|| {
                let threats = scanner
                    .scan_text(black_box(&injection_text))
                    .unwrap_or_default();
                black_box(threats);
            })
        });
...
fn latency_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("detect_invisible_unicode", |b| {
        ...
        b.iter(|| {
            ...
            assert!(threats
                .iter()
                .any(|t| matches!(t.threat_type, ThreatType::UnicodeInvisible)));
            black_box(threats);
        })
    });
    ...
    group.bench_function("detect_path_traversal", |b| {
        ...
        b.iter(|| {
            ...
            assert!(threats
                .iter()
                .any(|t| matches!(t.threat_type, ThreatType::PathTraversal)));
            black_box(threats);
        })
    });
