...
async fn draw_ui(frame: &mut Frame<'_>, metrics: &PerformanceMetrics) {
    ...
    for (threat_type, count) in threat_types.iter() {
        threat_lines.push(Line::from(vec![
            ...
            Span::styled(format!("{:>5}", count), Style::default().fg(Color::Red)),
        ]));
    }

    let threat_breakdown = Paragraph::new(threat_lines).block(
        Block::default()
...
async fn main() -> Result<()> {
    ...
    println!(
        "Threats detected: {}",
        metrics.threats_detected.load(Ordering::Relaxed)
    );
    println!(
        "Average throughput: {:.2} MB/s",
        metrics.get_average_throughput()
    );
    println!(
        "Average latency: {:.2} ms/file",
