    ...
    fn start_span(&self, name: &str) -> TelemetrySpan {
        let span = TelemetrySpan {
            ...
            attributes: vec![
                ...
                ("service.version".to_string(), self.config.service_version.clone()),
                ("telemetry.mode".to_string(), "enhanced".to_string()),
            ],
        };
        
        // Use advanced buffer for efficient span tracking
        self.buffer.track_span_start(&span.name, span.start_time.elapsed().as_nanos() as u64);
    ...
    fn record_metric(&self, metric: TelemetryMetric) {
        ...
        match metric.value {
            MetricValue::Counter(value) => {
                self.buffer.increment_counter(&metric.name, value, metric.labels);
            }
            MetricValue::Gauge(value) => {
                self.buffer.record_gauge(&metric.name, value, metric.labels);
            }
            MetricValue::Histogram(value) => {
                self.buffer.record_histogram(&metric.name, value, metric.labels);
            }
        }
    }
