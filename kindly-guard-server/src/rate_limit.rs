...
impl Default for RateLimitConfig {
    fn default() -> Self {
        ...
        method_limits.insert("tools/list".to_string(), MethodLimit {
            rpm: 60,
            burst: 10,
        });
        method_limits.insert("resources/list".to_string(), MethodLimit {
            rpm: 60,
            burst: 10,
        });
        
        // Lower limits for execution operations
        method_limits.insert("tools/call".to_string(), MethodLimit {
            rpm: 30,
            burst: 5,
        });
        
        // Very low limits for security-sensitive operations
        method_limits.insert("security/threats".to_string(), MethodLimit {
            rpm: 10,
            burst: 2,
        });
    ...
    pub async fn get_status(&self, client_id: &str) -> Result<HashMap<String, RateLimitResult>> {
        ...
        for (key, bucket) in buckets.iter() {
            if key.client_id == client_id {
                ...
                status.insert(method.to_string(), RateLimitResult {
