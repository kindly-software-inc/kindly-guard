...
impl DefaultPluginManager {
    ...
    pub fn new(config: PluginConfig) -> Result<Self> {
        let mut loaders: Vec<Box<dyn PluginLoader>> = vec![
            Box::new(NativePluginLoader::new()),
        ];
...
impl PluginManagerTrait for DefaultPluginManager {
    ...
    async fn load_plugin(&self, path: &Path) -> Result<PluginId> {
        ...
        for loader in &self.loaders {
            match loader.validate_plugin(path).await {
                Ok(metadata) => {
                    debug!("Plugin validated by {} loader: {}", loader.loader_type(), metadata.name);
        ...
        Err(anyhow::anyhow!("No loader could handle plugin at {:?}", path))
    }
        ...
        for (id, handle) in plugins.iter() {
            ...
            match tokio::time::timeout(timeout, scan_future).await {
                Ok(Ok(threats)) => {
                    ...
                    if !threats.is_empty() {
                        debug!("Plugin {} found {} threats", handle.metadata.name, threats.len());
                    }
    ...
    async fn scan(&self, id: &PluginId, context: ScanContext<'_>) -> Result<Vec<Threat>> {
        ...
        let result = tokio::time::timeout(timeout, scan_future).await
    ...
    async fn reload_plugin(&self, id: &PluginId) -> Result<()> {
        ...
        Err(anyhow::anyhow!("Hot reload not implemented for this plugin type"))
    }
}