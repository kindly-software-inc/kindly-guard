...
impl SigningManager {
    ...
    pub fn verify_message(&self, signed: &SignedMessage) -> Result<()> {
        ...
        if !self.config.require_signatures {
            let elapsed = SystemTime::now().duration_since(self.start_time)?.as_secs();
        ...
        let canonical = self.canonicalize_message(&signed.message, signed.signature.timestamp)?;
        ...
        match signed.signature.algorithm {
            SigningAlgorithm::HmacSha256 => {
                ...
                let expected = general_purpose::STANDARD.decode(&signed.signature.signature)?;
            ...
            SigningAlgorithm::Ed25519 => {
                ...
                let signature_bytes =
                    general_purpose::STANDARD.decode(&signed.signature.signature)?;
...
mod tests {
    ...
    fn test_hmac_signing() {
        let config = SigningConfig {
            ...
            algorithm: SigningAlgorithm::HmacSha256,
            hmac_secret: Some(
                general_purpose::STANDARD.encode(b"FAKE-TEST-KEY-DO-NOT-USE-IN-PROD-32b"),
            ),
