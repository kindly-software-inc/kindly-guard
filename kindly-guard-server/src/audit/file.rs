...
impl FileAuditLogger {
        ...
        if let Some(rotation) = &self.config.rotation {
            ...
            match rotation.strategy {
                RotationStrategy::Size => {
                    Ok(current_size >= rotation.max_size_mb * 1024 * 1024)
                }
                RotationStrategy::Time => {
                    Ok(age_hours >= rotation.max_age_hours)
                }
    ...
    async fn rotate(&self) -> Result<()> {
        ...
        *rotation_count += 1;
        ...
        let rotation_path = self.file_path.with_extension(format!("log.{}", rotation_count));
    ...
    async fn cleanup_old_files(&self, max_backups: u32) -> Result<()> {
        ...
        for (path, _) in rotation_files.iter().skip(max_backups as usize) {
            match std::fs::remove_file(path) {
                Ok(_) => debug!("Removed old audit log: {:?}", path),
