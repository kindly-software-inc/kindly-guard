...
async fn reload_config(
    ...
    let has_errors = all_errors.iter()
    ...
    async fn handle_change(&self, event: ConfigReloadEvent) -> Result<()> {
        match event {
            ConfigReloadEvent::Reloaded { changed_fields, .. } => {
                info!("Configuration reloaded. Changed fields: {:?}", changed_fields);
                ...
                let audit_event = AuditEvent::new(
                    AuditEventType::ConfigChanged {
                        ...
                        changes: changed_fields.into_iter()
                            .map(|f| (f.clone(), "updated".to_string()))
                            .collect(),
                    },
                    AuditSeverity::Info
                );
