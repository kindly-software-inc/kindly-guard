...
static SAFE_NAME_PATTERN: Lazy<Regex> = Lazy::new(|| {
...
pub fn validate_scan_input(input: &str) -> Result<String> {
    ...
    if input.len() > MAX_SCAN_INPUT_SIZE {
        bail!("Input too large: maximum {} MB", MAX_SCAN_INPUT_SIZE / 1024 / 1024);
    }
...
pub fn validate_port(port: u16) -> Result<u16> {
    if port < MIN_USER_PORT {
        bail!("Port {} is reserved. Use ports {} and above", port, MIN_USER_PORT);
    }
...
mod tests {
    ...
    fn test_feature_name_validation() {
        ...
        assert_eq!(validate_feature_name("custom-feature").unwrap(), "custom-feature");
    ...
    fn test_output_sanitization() {
        ...
        assert_eq!(sanitize_output("Line 1\nLine 2\tTabbed"), "Line 1\nLine 2\tTabbed");
        
        // Preserve ANSI colors
        assert_eq!(sanitize_output("\x1b[31mRed text\x1b[0m"), "\x1b[31mRed text\x1b[0m");
