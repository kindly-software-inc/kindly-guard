...
pub fn confirm_action(action: &str, path: &str) -> Result<bool> {
    ...
    print!("{} {} ", "Proceed?".yellow().bold(), "[Y/n]".dark_grey());
    io::stdout().flush()?;
    ...
    if success {
        ...
        match ide {
            IdeType::VsCode | IdeType::Cursor => {
                ...
                println!(
                    "  2. {} {}",
                    "Open the MCP panel".yellow(),
                    "(View → MCP Servers)".dark_grey()
                );
                println!(
                    "  3. {} {}",
                    "KindlyGuard should appear as".yellow(),
                    "\"kindly-guard\"".green()
                );
            },
            IdeType::Zed => {
                ...
                println!(
                    "  2. {} {}",
                    "Check Assistant panel for".yellow(),
                    "KindlyGuard".green()
                );
            },
            IdeType::Neovim => {
                ...
                println!(
                    "  2. {} {}",
                    "KindlyGuard MCP server is now available to".yellow(),
                    "LSP clients".cyan()
                );
            },
        ...
        println!(
            "  • {} {}",
            "Check permissions for".yellow(),
            "configuration directory".cyan()
        );
        println!(
            "  • {} {}",
            "Ensure".yellow(),
            "kindly-guard is in your PATH".cyan()
        );
        println!(
            "  • {} {}",
            "Try running with".yellow(),
            "--debug flag".cyan()
        );
    }
...
pub fn print_welcome_banner() {
    ...
    println!(
        "{} {} {}",
        "│".dark_grey(),
        "    KindlyGuard MCP Setup    ".green().bold(),
        "│".dark_grey()
    );
    println!("{}", "╰─────────────────────────────────────╯".dark_grey());
    println!();
    println!(
        "{}",
        "This wizard will help you configure KindlyGuard".cyan()
    );
