...
impl CliShield {
    ...
    fn render_compact(&self, status: &ShieldStatus) -> String {
        ...
        if threat_count > 0 {
            format!(
                "[{} KindlyGuard: {} Protected | ⚡ {} blocked | ⏱ {}]",
                shield_icon, status_icon, threat_count, uptime
            )
        } else {
            format!(
                "[{} KindlyGuard: {} Protected | ⏱ {}]",
                shield_icon, status_icon, uptime
            )
        }
    }
    ...
    fn render_status_bar(&self, status: &ShieldStatus) -> String {
        ...
        if let Some(last_threat) = &status.last_threat {
            ...
            format!("{} Safe ⚡{}", shield_icon, threats)
        }
    }
    ...
    fn write_display(&self, display: &str) -> io::Result<()> {
        ...
        match self.format {
            DisplayFormat::Inline => {
                ...
                write!(stdout, "\x1b[1;{}H", terminal::size()?.0.saturating_sub(display.len() as u16))?;
...
pub mod hooks {
    ...
    pub fn pre_command_hook() -> &'static str {
        r#"
    ...
    pub fn post_command_hook() -> &'static str {
        r#"
        if command -v kindly-guard >/dev/null 2>&1; then
    ...
    pub fn prompt_command() -> &'static str {
        r#"
        if command -v kindly-guard >/dev/null 2>&1; then
