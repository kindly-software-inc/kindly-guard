...
impl ShieldDisplay {
    ...
    pub fn new(shield: Arc<Shield>, config: ShieldConfig) -> Self {
        Self { shield, config }
    }
    ...
    fn draw_ui(&self, frame: &mut Frame) {
        ...
        // Threat statistics
        let mut threat_items = vec![
            Line::from("Threats Blocked by Type:"),
            Line::from(""),
        ];
        
        // Count threats by category
        let unicode_count: u64 = threat_stats.iter()
            .filter(|(k, _)| matches!(k, ThreatType::UnicodeInvisible | ThreatType::UnicodeBiDi | ThreatType::UnicodeHomograph | ThreatType::UnicodeControl))
            .map(|(_, v)| v)
            .sum();
            
        let injection_count: u64 = threat_stats.iter()
            .filter(|(k, _)| matches!(k, ThreatType::PromptInjection | ThreatType::CommandInjection | ThreatType::SqlInjection))
            .map(|(_, v)| v)
            .sum();
            
        let traversal_count = threat_stats.get(&ThreatType::PathTraversal).copied().unwrap_or(0);
        
        let mcp_count: u64 = threat_stats.iter()
            .filter(|(k, _)| matches!(k, ThreatType::SessionIdExposure | ThreatType::ToolPoisoning | ThreatType::TokenTheft))
            .map(|(_, v)| v)
            .sum();
        
        threat_items.push(Line::from(format!("├─ Unicode Attacks:     {}", unicode_count)));
        threat_items.push(Line::from(format!("├─ Injection Attempts:  {}", injection_count)));
        threat_items.push(Line::from(format!("├─ Path Traversal:      {}", traversal_count)));
