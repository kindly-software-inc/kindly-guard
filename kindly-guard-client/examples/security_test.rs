...
use kindly_guard_client::{
    AuthScenario, ClientConfiguration, SecurityTestRunner, SecurityTester, TestClient, ThreatType,
};
use std::io::Write;
use std::sync::Arc;
use tracing::info;
...
"#,
        ...
        info!(
            ...
            capabilities.security_features.enhanced_mode
        );
        ...
        let sql_threat = test_runner
            ...
            .await?;
        info!(
            "SQL Injection - Detected: {}, Shield: {:?}",
            sql_threat.threat_detected, sql_threat.shield_color
        );

        // Unicode attack
        let unicode_threat = test_runner
            .inject_threat(ThreatType::UnicodeAttack, "Normal\u{202E}Reversed")
            .await?;
        info!(
            "Unicode Attack - Detected: {}",
            unicode_threat.threat_detected
        );

        // Custom threat
        let custom_threat = test_runner
            .inject_threat(
        ...
        info!(
            "Rate limiting - Sent: {}, Allowed: {}, Blocked: {}",
            rate_result.requests_sent, rate_result.requests_allowed, rate_result.requests_blocked
        );
        ...
        for (scenario, name) in auth_scenarios {
            ...
            info!(
                "{} - Authenticated: {}, Error: {:?}",
                name, auth_result.authenticated, auth_result.error_code
            );
        }
