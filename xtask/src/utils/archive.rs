...
pub fn create_tar_gz(archive_path: &Path, source_dir: &Path, options: CreateOptions) -> Result<()> {
...
pub fn create_zip(archive_path: &Path, source_dir: &Path, options: CreateOptions) -> Result<()> {
    let file = File::create(archive_path)
        .with_context(|| format!("Failed to create archive: {}", archive_path.display()))?;
...
pub fn extract_tar_gz(archive_path: &Path, dest_dir: &Path, options: ExtractOptions) -> Result<()> {
...
pub fn extract_zip(archive_path: &Path, dest_dir: &Path, options: ExtractOptions) -> Result<()> {
    let file = File::open(archive_path)
        .with_context(|| format!("Failed to open archive: {}", archive_path.display()))?;
...
pub fn extract_file(archive_path: &Path, file_name: &str, dest_path: &Path) -> Result<()> {
    let extension = archive_path
        .extension()
        .and_then(|e| e.to_str())
        .unwrap_or("");
...
fn extract_file_from_tar_gz(archive_path: &Path, file_name: &str, dest_path: &Path) -> Result<()> {
    let file = File::open(archive_path)?;
    let decoder = GzDecoder::new(file);
    let mut archive = TarArchive::new(decoder);
...
fn extract_file_from_zip(archive_path: &Path, file_name: &str, dest_path: &Path) -> Result<()> {
    let file = File::open(archive_path)?;
    let mut archive = ZipArchive::new(BufReader::new(file))?;
