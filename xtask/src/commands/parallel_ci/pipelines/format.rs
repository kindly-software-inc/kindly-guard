...
impl Pipeline for FormatPipeline {
    ...
    async fn execute(
        ...
        send_progress(&event_tx, pipeline_name, 1, 3, "Checking code formatting...".to_string()).await;
        ...
        // Step 2: clippy
        send_progress(&event_tx, pipeline_name, 2, 3, "Running clippy lints...".to_string()).await;
        ...
        let clippy_result = Command::new("cargo")
            .args(&[
                ...
                "-W", "clippy::all",
                "-W", "clippy::pedantic",
                "-A", "clippy::missing_errors_doc", // Allow for now
                "-A", "clippy::module_name_repetitions", // Allow for now
            ])
            .output()
            .await?;
        ...
        send_progress(&event_tx, pipeline_name, 3, 3, "Checking dependencies...".to_string()).await;
