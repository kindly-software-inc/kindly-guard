...
use crate::utils::{Context, nextest};
    ...
    async fn execute(
        ...
        match self.mode {
            TestMode::SmokeTests => {
                ...
                send_progress(&event_tx, pipeline_name, 1, 3, "Testing binary execution...".to_string()).await;
                ...
                send_progress(&event_tx, pipeline_name, 2, 3, "Running quick unit tests...".to_string()).await;
                ...
                let unit_test = Command::new("cargo")
                    .args(&test_cmd)
                    .output()
                    .await?;
                ...
                send_progress(&event_tx, pipeline_name, 3, 3, "Testing MCP server startup...".to_string()).await;
                
                // Start server and immediately shut it down
                let server_test = tokio::time::timeout(
            ...
            TestMode::FullSuite => {
                ...
                // Step 1: Unit tests
                send_progress(&event_tx, pipeline_name, 1, total_steps, "Running unit tests...".to_string()).await;
                ...
                send_progress(&event_tx, pipeline_name, 2, total_steps, "Running integration tests...".to_string()).await;
                ...
                let integration_test = Command::new("cargo")
                    .args(&test_cmd)
                    .output()
                    .await?;
                ...
                // Step 3: Doc tests
                send_progress(&event_tx, pipeline_name, 3, total_steps, "Running doc tests...".to_string()).await;
                ...
                send_progress(&event_tx, pipeline_name, 4, total_steps, "Running property tests...".to_string()).await;
                ...
                send_progress(&event_tx, pipeline_name, 5, total_steps, "Generating test coverage...".to_string()).await;
                ...
                if coverage_available {
                    ...
                    let _ = Command::new("cargo")
                        .args(&["llvm-cov", "--lcov", "--output-path", ".ci/reports/coverage.lcov"])
                        .output()
                        .await;
