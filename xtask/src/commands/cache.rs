...
fn setup_local_cache(ctx: &Context, max_size: &str) -> Result<()> {
    ctx.info(&format!(
        "Configuring local cache with max size: {}",
        max_size
    ));
...
fn setup_gha_cache(ctx: &Context) -> Result<()> {
    ...
    env::set_var(
        ...
        env::var("ACTIONS_RUNTIME_TOKEN").unwrap_or_default(),
    );
...
async fn show_stats(ctx: Context) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        Command::new("sccache").arg("--start-server").output()?;
...
fn load_cache_config() -> Result<CacheConfig> {
    ...
    Ok(CacheConfig {
        ...
        cache_dir: config["cache"]
            ...
            .and_then(|v| v.as_str())
            .map(String::from),
        s3_bucket: config["cache"]
            .get("bucket")
            .and_then(|v| v.as_str())
            .map(String::from),
        redis_url: config["cache"]
            .get("url")
            .and_then(|v| v.as_str())
            .map(String::from),
    })
